...
async def client() -> AsyncIterator[httpx.AsyncClient]:
    """Provide an authenticated-test-ready app backed by a per-test SQLite file."""
    with tempfile.TemporaryDirectory(prefix="multiscribe-api-") as tmp:
        settings = SystemSettings(_env_file=None, db_path=str(Path(tmp) / "api.sqlite"))
