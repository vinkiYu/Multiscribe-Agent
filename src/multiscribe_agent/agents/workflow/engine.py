...
class WorkflowEngine:
    ...
    async def run(
        self, workflow_id: str, input_data: object, *, date: str | None = None
    ) -> dict[str, object]:
        """Return completed step outputs and final output."""
        outputs: dict[str, object] = {}
        final = input_data
        async for event in self.stream(workflow_id, input_data, date=date):
    ...
    async def stream(
        ...
        yield WorkflowEvent("workflow_start", {"workflow_id": workflow_id}, trace_id)
        try:
        ...
        for level in levels:
            ...
            for (step, _), outcome in zip(executions, outcomes, strict=True):
                ...
                if self._is_empty(output) and graph.successors[step.id]:
                    message = f"step {step.id} produced an empty output with downstream successors"
    ...
    async def _execute_step(
        self, step: WorkflowStep, value: object, trace_id: str
        ...
        if step.step_type == "workflow":
            ...
            return (await self.run(step.workflow_id, value))["final"], []
