"""Last Week in AI RSS adapter.

Subclass of :class:`RSSAdapter` that tags items with a dedicated source name
and ``metadata.id`` so the curation pipeline can attribute Last Week in AI
newsletter entries. The endpoint serves an RSS 2.0 feed at
...
DEFAULT_FEED_URL = "https://www.lastweekin.ai/feed"
DEFAULT_SOURCE_NAME = "Last Week in AI"


class LastWeekInAIAdapter(RSSAdapter):
    """Fetch Last Week in AI newsletter entries with a stable source."""

    metadata: ClassVar[PluginMetadata] = PluginMetadata(
        id="last_week_in_ai",
        type="adapter",
        name="Last Week in AI",
        description="Fetch Last Week in AI newsletter entries summarizing AI industry news.",
        icon="article",
        config_fields=[
            ConfigField(
                key="rss_url",
                ...
                required=False,
                scope="item",
                help_text="Override the default Last Week in AI RSS feed URL.",
            ),
            ConfigField(
                key="source_name",
                label="Source name",
    ...
    async def fetch_and_transform(self, config: Mapping[str, object]) -> list[UnifiedData]:
        """Apply Last Week in AI defaults when callers omit URL or label."""
    ...
    def transform(
        ...
        """Apply Last Week in AI defaults before delegating to ``RSSAdapter``."""
