...
5. **遇歧义停下问**：任务包描述不清、发现契约缺字段、测试无法满足验收时——**停下报告 BLOCKED**，禁止在 review 里悄悄猜测实现。
   - **契约缺口处理流程**：若发现前置包(Px-1)的领域模型/配置缺了本包需要的字段，**不要擅自改前置包的文件**（那是黑名单）。保持工作区干净，在 review 里写明：(a) 缺什么字段、(b) 在哪个前置文件、(c) 本包为什么需要、(d) 建议的 2-3 个方案。规划会决定改前置包（规划层修复）还是调整本包契约。P3 的 `ProviderConfig.models` 就是此流程的成功案例。
