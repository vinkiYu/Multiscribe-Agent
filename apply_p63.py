"""Insert AgentsConfigPage into prototype/src/shared/ui.tsx."""
# encoding: utf-8
import re
...
agents = '''function AgentsConfigPage(): ReactElement {
  ...
  const [settingsRemote] = useState(() => ({ data: null as any }))
  ...
  useEffect(() => { void settingsApi.get().then((data) => (settingsRemote as any).data = data).catch(() => undefined) }, [])
  useEffect(() => {
    if (remote.data) {
  ...
  const settings = (settingsRemote as any).data as any
  ...
  const tryRun = async () => {
    ...
    setTrying(true)
    setTryEvents([])
    const streamingId = `try-streaming-${Date.now()}`
    try {
      ...
      // streamingId marker is intentionally unused (P62.3 W2 pattern ref)
      void streamingId
    } catch (caught) {
...
# Insert AgentsConfigPage right before the trailing exports block
marker = '  SettingsPageV2,\n  SettingsPageV3,'
if marker not in content:
    raise SystemExit('marker not found')
if 'function AgentsConfigPage' in content:
    print('Already inserted; skipping')
else:
    content = content.replace(marker, agents + '  ' + marker, 1)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print('Inserted AgentsConfigPage')
