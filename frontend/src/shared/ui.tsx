...
function SourceConfigFieldControl({ field, value, onChange }: { field: SourceConfigField; value: unknown; onChange: (value: unknown) => void }): ReactElement {
  ...
  if (field.type === 'boolean') return <Switch checked={Boolean(value ?? field.default)} onCheckedChange={onChange} label={<span><strong>{label}</strong><small>{help}</small></span>} />
...
function ProviderConfigurationCard({ provider, onChange }: { provider: any; onChange: (provider: any) => void }): ReactElement {
  ...
  return <article className={`provider-card provider-config-card ${provider.enabled ? '' : 'is-disabled'}`}>
    <header className="provider-card-header">
      ...
      <div className="provider-actions"><button type="button" className="test-connection-button" disabled={testing} onClick={() => void testConnection()}><RadioTower className={testing ? 'spinning' : ''} />{testing ? '测试中…' : '测试连接'}</button><Switch checked={provider.enabled} onCheckedChange={(enabled) => update({ enabled })} label={provider.enabled ? '已激活' : '已停用'} /></div>
    </header>
    <div className="provider-config-body">
      ...
      <section className="provider-models">
        ...
        <section className="provider-catalog"><h3>模型列表</h3><MultiSelectDropdown triggerLabel={catalog ? `已同步 ${catalog.length} 个模型` : '请先同步列表'} items={catalog?.map((model) => ({ value: model, label: model, checked: provider.models.includes(model) })) ?? []} onCheckedChange={selectModel} disabled={!catalog} ariaLabel="同步得到的模型列表" /></section>
      </section>
    </div>
  </article>
}
