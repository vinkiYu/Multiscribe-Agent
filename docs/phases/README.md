...
| P53-D | 性能优化（save_batch 去掉双 COUNT(*) + _recent_daily_candidates 查询合并 + SQL audit 批量写）| 🔄 进行中 | 任务包已就绪，待 Codex 执行；6 条验收 |
| P53-E | 文档补给（API Reference + Configuration Reference + Deployment Guide + Troubleshooting Guide）| 🔄 进行中 | 任务包已就绪，待 Codex 执行；5 条验收；可与 P53-D 并行 |
