...
| P53-A | upsert 抽象层（dialect.py 新增 UpsertStyle + render_upsert + _upsert_sql；迁移 4 个最高风险文件：curation_evaluations + daily_usage + kv + iteration_store 走 mixin）| 🔄 进行中 | 任务包已就绪，待 Codex 执行；15 条验收；剩余 9 个硬编码 ON CONFLICT 文件标 TODO 后续包处理 |
